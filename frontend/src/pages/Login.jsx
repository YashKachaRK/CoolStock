...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    // Hardcoded logic from original JSP with an exception for new customers
    if (role === 'admin' && username === 'admin' && password === '1234') {
      navigate('/admin/dashboard');
    } else if (role === 'manager' && username === 'manager' && password === '1234') {
      navigate('/manager/dashboard');
    } else if (role === 'delivery' && username === 'delivery' && password === '1234') {
      navigate('/delivery/dashboard');
    } else if (role === 'cashier' && username === 'cashier' && password === '1234') {
      navigate('/cashier/dashboard');
    } else if (role === 'customer' && (username === 'customer' || password !== '')) {
      // Allowing any customer login with any credential for demo purposes (mostly after registration)
      navigate('/customer/place_order');
    } else {
  ...
  const handleRegister = (e) => {
    ...
    localStorage.setItem('cs_profile_cust1', JSON.stringify(newCustomerProfile));
    ...
    <div className="bg-gradient-to-br from-purple-600 to-pink-500 flex items-center justify-center min-h-screen p-4 w-full">
      <div className={`bg-white p-8 rounded-3xl shadow-2xl w-full transition-all duration-300 ${isRegistering ? 'max-w-2xl' : 'max-w-md'}`}>
        <div className="text-center mb-7">
          ...
          <p className="text-gray-400 text-sm mt-1">Ice Cream Wholesale Management</p>
        </div>
        ...
        {!isRegistering ? (
          ...
          <>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-600 mb-1">Select Role</label>
                <select 
                  value={role}
            ...
            <div className="mt-6 bg-gray-50 rounded-xl p-4 text-xs text-gray-400 space-y-1">
              <p className="font-semibold text-gray-500 mb-2">🧪 Demo Credentials (all passwords: 1234)</p>
          ...
          <>
            <div className="mb-6 text-center">
              <h2 className="text-xl font-bold text-gray-800">🏪 Customer Registration</h2>
              <p className="text-gray-500 text-sm">Create an account to order ice cream in bulk.</p>
            </div>
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    type="text" required placeholder="e.g. Ramesh General Store" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="email" required placeholder="e.g. ramesh@gmail.com" 
                ...
                <div>
                  ...
                  <input 
                    type="tel" required placeholder="e.g. +91 94001 11111" 
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-600 mb-1">Complete Shop Address</label>
                <textarea 
                  required rows="2" placeholder="e.g. Village Khari, Dist. Anand" 
        ...
        <div className="mt-4 text-center">
          <Link to="/" className="text-purple-500 text-sm hover:underline font-semibold">
            ← Back to Home
          </Link>
        </div>
      </div>
