              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50 transition">
                    <td className="py-3 px-5 font-bold text-slate-700">{order.id}</td>
                    <td className="px-5">
                      <p className="font-semibold">{order.customer}</p>
                      <p className="text-xs text-gray-400">{order.addr}</p>
                    </td>
                    <td className="px-5 text-xs text-gray-500 max-w-[180px] truncate">{order.items}</td>
                    <td className="px-5 font-bold text-purple-700">₹{order.amount.toLocaleString('en-IN')}</td>
                    <td className="px-5 text-gray-400 text-xs">{order.date}</td>
                    <td className="px-5">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-0 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-slate-800 to-purple-700 text-white p-7">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h2 className="text-2xl font-black mt-0.5">{viewOrder.id}</h2>
                  <p className="text-sm opacity-80 mt-1">{viewOrder.date}</p>
                </div>
            ...
            <div className="p-7 space-y-4">
              <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
                ...
                <p className="font-black text-gray-800 text-lg">{viewOrder.customer}</p>
                <p className="text-sm text-gray-500">📍 {viewOrder.addr}</p>
                <p className="text-sm text-gray-500">📞 {viewOrder.phone}</p>
              </div>
              <div className="bg-gray-50 rounded-2xl p-4">
                <p className="text-xs font-bold text-gray-400 uppercase mb-2">Items Ordered</p>
                <p className="text-sm text-gray-700">{viewOrder.items}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-purple-50 rounded-2xl p-4">
                  <p className="text-xs font-bold text-gray-400 uppercase">Order Amount</p>
                  <p className="text-2xl font-black text-purple-700 mt-1">₹{viewOrder.amount.toLocaleString('en-IN')}</p>
                </div>
                <div className="bg-gray-50 rounded-2xl p-4">
                  ...
                  {viewOrder.deliveryBoy && <p className="text-xs text-gray-500 mt-1">🛵 {viewOrder.deliveryBoy}</p>}
                </div>
              </div>
