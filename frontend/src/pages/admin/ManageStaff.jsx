...
export default function ManageStaff() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/addStaff", newStaff);

      if (res.status === 200) {
        showToast("🎉 Staff added to database!");
        setAddModal(false);
    ...
    <div className="p-4 md:p-8 w-full">
      ...
      <div className="bg-gradient-to-r from-slate-800 to-blue-700 text-white p-5 md:p-7 rounded-2xl mb-6 md:mb-8 flex justify-between items-center shadow-lg">
        <div>
          ...
          <p className="opacity-80 mt-1 text-sm">
            View, add, and manage all CoolStock employees
          </p>
        </div>
        <button
            ...
            <tbody className="text-gray-700 divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                filtered.map((s) => (
                  <tr
                    key={s.id}
                    className={`hover:bg-gray-50 transition ${s.status === "Inactive" ? "opacity-60" : ""}`}
                  >
                    ...
                    <td className="px-6">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-bold ${s.status === "Active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                      >
      ...
      {viewStaff && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setViewStaff(null);
          }}
        >
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-gradient-to-r from-slate-800 to-blue-700 text-white p-7 text-center">
              ...
              <span
                className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold bg-white/20`}
              >
                {ROLE_COLORS[viewStaff.role] ? viewStaff.role : viewStaff.role}
              </span>
            </div>
            <div className="p-7 space-y-3">
              {[
                ["👤 Username", viewStaff.username],
                ["📧 Email", viewStaff.email],
                ["📞 Phone", viewStaff.phone],
                ["📅 Joined", viewStaff.joined],
                ["⚡ Status", viewStaff.status],
              ].map(([label, val]) => (
                <div
                  key={label}
                  className="flex justify-between items-center bg-gray-50 rounded-xl px-4 py-2.5"
                >
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="email"
                  placeholder="suresh@coolstock.in"
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={handleAdd}
                className="flex-1 py-3 bg-gradient-to-r from-slate-700 to-blue-600 text-white font-bold rounded-2xl hover:opacity-90 transition shadow-lg"
              >
                🎉 Add Staff
              </button>
              <button
